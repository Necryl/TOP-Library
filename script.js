...
formBtnElement.addEventListener("click", (event) => {
  let [titleElem, authorElem, pagesElem] = formInputElements;

  let proceed = true;
  [titleElem, authorElem, pagesElem].forEach((element) => {
    if (!notEmpty(element)) {
      if (proceed) {
        element.focus();
...
formInputElements.slice(0, 3).forEach((element) => {
...
function notEmpty(elem) {
  if (elem.value === "") {
    elem.setCustomValidity("This field cannot be empty!");
    return false;
  } else {
...
function createBookElem(title, author, pages, read) {
  ...
  result.childNodes[1].addEventListener("click", (event) => {
    removeBook(Number(result.childNodes[1].getAttribute("libIndex")));
  });
...
function askBookDetails() {
  ...
  params.push(
    confirm(
      "If you have finished reading this book then press OK, otherwise press Cancel"
    )
  );
...
function removeBook(libIndex) {
  library[libIndex].element.remove();
...
function updateStats() {
  ...
  statsElements.forEach((element, index) => {
    ...
    element.textContent = result[0] + ": " + stats[index];
  });
}
